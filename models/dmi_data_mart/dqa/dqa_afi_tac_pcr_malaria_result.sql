...
with raw_counts as (
    ...
    select
        "TACmalariaResult" as tac_malaria_result,
        count(*) as raw_value
    ...
    where enr_interviewdate between '{{ start_date }}' and '{{ end_date }}'
      and consent = '1'
      and "TACmalariaResult" in ('Positive', 'Negative')
...
reporting_counts as (
    ...
    from {{ ref('aggregate_afi_surveillance_malaria_tac_pcr_report') }}

    where screening_date between '{{ start_date }}' and '{{ end_date }}'
      and tac_malaria_result in ('Positive', 'Negative')
...
powerbi_counts as (
    ...
    from (
        values
            ('Positive'::text, 2453::bigint),
            ('Negative'::text, 4691::bigint)

    ) as p(tac_malaria_result, powerbi_value)
...
comparison as (
    ...
    left join raw_counts r
        on r.tac_malaria_result = p.tac_malaria_result

    left join staging_counts s
        on s.tac_malaria_result = p.tac_malaria_result

    left join reporting_counts rep
        on rep.tac_malaria_result = p.tac_malaria_result
...
select
    ...
    staging_value - reporting_value as staging_vs_reporting_variance,
    reporting_value - powerbi_value as reporting_vs_powerbi_variance,

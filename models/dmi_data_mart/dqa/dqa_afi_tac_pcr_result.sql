...
with raw_counts as (
    ...
    where enr_interviewdate between '{{ start_date }}' and '{{ end_date }}'
      and consent = '1'
      and "TacResult" in ('Positive', 'Negative')
...
reporting_counts as (
    ...
    where screening_date between '{{ start_date }}' and '{{ end_date }}'
      and tac_result in ('Positive', 'Negative')
    group by 1
),
powerbi_counts as (
    select *
    from (
        values
            ('Positive'::text, 3081::bigint),
            ('Negative'::text, 4063::bigint)

    ) as p(tac_result, powerbi_value)
),
comparison as (
    select
        p.tac_result,
    ...
    left join raw_counts r
        on r.tac_result = p.tac_result

    left join staging_counts s
        on s.tac_result = p.tac_result

    left join reporting_counts rep
        on rep.tac_result = p.tac_result
